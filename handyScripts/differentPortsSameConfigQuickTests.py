...
def connect_ports_as_per_quicktest_and_media(rfc_test, ixNetwork, iface_assignement_map):
    try:
        ...
        if ports_to_connect or ports_to_disconnect:  
            return True , ports_to_connect, ports_to_disconnect
        return False, ports_to_connect, ports_to_disconnect
    except Exception as e:
        raise Exception(str(e))
        
def releasePorts(ixNetwork, ports='all'):
        """
        Release all ports or release specified ports in the param ports.
        
        :param ports: 
        ports = list: [[ixChassisIp, 1, 2], [ixChassisIp, 1, 3], ...],
        ...
        for port in ports:
            ...
            vport = ixNetwork.Vport.find(AssignedTo=regexString)
            if vport:
